...
| 偏移 | 字段 | 长度 | 含义与典型值 |
|------|------|------|-------------|
| 0 | **Hardware Type (HTYPE)** | 2 B | 硬件类型。`1` = Ethernet（最常见）；`6` = IEEE 802；`15` = Frame Relay |
| 2 | **Protocol Type (PTYPE)** | 2 B | 上层协议类型，取值同 EtherType。`0x0800` = IPv4 |
| 4 | **Hardware Addr Length (HLEN)** | 1 B | 硬件地址长度。以太网 = `6` |
| 5 | **Protocol Addr Length (PLEN)** | 1 B | 协议地址长度。IPv4 = `4` |
| 6 | **Operation (OPER / opcode)** | 2 B | 操作码：`1` = ARP Request，`2` = ARP Reply，`3` = RARP Request，`4` = RARP Reply，`8`/`9` = InARP |
| 8 | **Sender Hardware Address (SHA)** | 6 B | **发送方 MAC**。接收方据此学习映射 |
| 14 | **Sender Protocol Address (SPA)** | 4 B | **发送方 IP**。与 SHA 构成一条映射记录 |
| 18 | **Target Hardware Address (THA)** | 6 B | 目标 MAC。请求时填 `00:00:00:00:00:00`（未知）；应答时填请求方 MAC |
| 24 | **Target Protocol Address (TPA)** | 4 B | **目标 IP**。请求时 = 待解析的 IP；应答时 = 请求方 IP |
...
> **判读口诀**：`SHA/SPA` 永远是"我是谁"，`THA/TPA` 永远是"我找谁"。三种特殊报文全靠这四个字段的组合区分（见第 4 节）。
