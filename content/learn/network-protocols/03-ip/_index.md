...
>
> **初学者最常踩的坑**：把 IP 当成"能保证送到"的协议。IP 明确**不保证送达、不保证顺序、不保证不重复**，出错就直接丢弃——可靠性是 TCP 在端系统上另外补出来的。第二个高频坑是忘了"IP 只决定下一跳是谁"：算出下一跳之后，还必须由 ARP（IPv4）/ NDP（IPv6）把它翻译成 MAC，帧才发得出去。
