...
**初学者最常踩的坑**：以为"ping 不通 = 网络断了"。其实 ping 被禁（`icmp_echo_ignore_all=1`）、被限速、被防火墙 DROP，或者回程路由缺失，都会表现为"不通"，但三层/四层业务可能好好的——此时要用 `nc`/`curl`/`traceroute -T` 做四层或 TCP 层验证。另一个坑：以为 ICMP 会重传，实际上它**不可靠、丢了就丢了**，所以 traceroute 中间跳偶发 `* * *` 不代表链路故障。
