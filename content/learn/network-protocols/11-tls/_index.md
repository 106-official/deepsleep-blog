...
>
> ⚠️ 初学者最常踩的坑：以为抓包里 Record Layer 显示的 `0x0303` 就是真实协商版本。TLS 1.3 为了穿透中间设备，会把记录层版本固定伪装成 1.2 的 `0x0303`，真实版本藏在 `supported_versions` 扩展里——不展开扩展看，就会误判"服务器不支持 1.3"。
>
> ⚠️ 另一个高频坑：把"SSL 证书"当成一种独立于 TLS 的东西。SSL 2.0/3.0 早已被 RFC 6176 / RFC 7568 禁用，今天所有所谓"SSL 证书"跑的都是 TLS，只是叫法沿用了历史。
