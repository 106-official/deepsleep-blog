...
把两个城市的分公司想象成两栋办公楼。以前员工之间寄快递，包裹是透明的，路上谁都能看、能换里面的东西。IPSec 相当于在两栋楼之间开通了一条**装甲运钞车专线**：所有寄往对方楼的包裹，先统一装进不透明的保险箱、贴上防拆封条，再由运钞车送过去——**寄件的员工什么都不用改，照常写地址扔进收发室就行**。

而"隧道模式"更形象：连原来的快递单（原始 IP 头）都一起塞进保险箱，外面只贴两栋楼之间的地址。路上的人只知道"A 楼在给 B 楼送东西"，却不知道具体是哪个部门寄给哪个部门的。
...
1. 【一族而非一个】**协议族而非单一协议**：由「两个安全协议（AH、ESP）+ 一个密钥管理协议（IKE）+ 三个数据库（SPD、SAD、PAD）」组成。
2. 【换内衣 vs 套外套】**两种封装模式**：
   - **传输模式（Transport Mode）**：保留原 IP 头，只保护载荷。用于**主机到主机**直连。
   - **隧道模式（Tunnel Mode）**：把整个原始 IP 包当作载荷，外面再套一个新 IP 头。用于**网关到网关**，是 VPN 的标准做法。
3. 【SA 单向】**安全关联（SA）是单向的**：一次双向通信需要**两个 SA**（进方向 + 出方向）。SA 由三元组 `<SPI, 目的 IP, 安全协议(AH/ESP)>` 唯一标识。
4. 【策略驱动】**策略驱动**：SPD 是"该不该保护"，SAD 是"用什么参数保护"。数据包先查 SPD 决定动作，再查 SAD 取出密钥与算法。
5. 【先谈信道再谈数据】**两阶段密钥协商**：IKEv2 先建 IKE SA（保护协商信道本身），再在其保护下建 Child SA（实际保护数据的 ESP/AH SA）。
6. 【IPv6 原生】**IPv6 原生支持**：IPSec 在 IPv6 中作为扩展头存在，设计上比 IPv4 更自然（虽然 RFC 6434 已将其从"必须实现"降为"应当实现"）。
...
>
> ⚠️ 初学者最常踩的坑：以为"隧道显示 UP 了就等于通了"。实际上 SA 建立成功只代表控制平面谈妥了，数据平面还可能因为路由没指过去、`ip_forward` 没开、NAT 规则抢先把内网流量 SNAT 导致不命中 SPD 而完全不通——这也是本目录 02 篇里出现频率最高的故障。
>
> ⚠️ 另一个高频坑：在有 NAT 的环境里选了 AH。AH 的 ICV 覆盖 IP 头中的源/目的地址，NAT 一改地址校验必然失败，NAT 环境下只能用 ESP 并启用 NAT-T（UDP 4500）。
