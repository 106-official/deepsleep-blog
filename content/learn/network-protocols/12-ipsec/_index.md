...
>
> 初学者最常踩的坑：以为"隧道显示 UP 了就等于通了"。实际上 SA 建立成功只代表控制平面谈妥了，数据平面还可能因为路由没指过去、`ip_forward` 没开、NAT 规则抢先把内网流量 SNAT 导致不命中 SPD 而完全不通——这也是本目录 02 篇里出现频率最高的故障。
>
> 另一个高频坑：在有 NAT 的环境里选了 AH。AH 的 ICV 覆盖 IP 头中的源/目的地址，NAT 一改地址校验必然失败，NAT 环境下只能用 ESP 并启用 NAT-T（UDP 4500）。
