...
>
> ⚠️ 初学者最常踩的坑：把 `Cache-Control: no-cache` 理解成"不要缓存"。它的真实含义是"可以缓存，但每次用之前必须回服务器校验"；真正禁止存储的是 `no-store`。这一字之差在处理密码、支付等敏感响应时后果完全不同。
>
> ⚠️ 另一个高频坑：以为升级到 HTTP/2 就彻底告别队头阻塞了。HTTP/2 只解决了**应用层**的队头阻塞，底层 TCP 一旦丢包，所有流仍会一起卡住——这才是 HTTP/3 转投 QUIC 的根本原因。
