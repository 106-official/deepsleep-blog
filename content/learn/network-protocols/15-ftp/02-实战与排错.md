...
| 现象 | 可能原因 | 排查方法 |
|------|---------|---------|
| **425 Failed to establish connection** / **425 Can't open data connection** | 数据连接建不起来：主动模式下客户端在 NAT 后；被动模式下服务器被动端口未放行 | 先 `passive` 切被动；服务端确认 `pasv_min_port/max_port` 已在防火墙与安全组放行；`nmap -p 40000-40100` 验证 |
| 能登录、能 `pwd`，但 **`ls` 就卡住** | 典型的数据连接问题——`LIST` 也需要数据连接 | 同上。这是"控制通、数据不通"的最经典症状 |
| **227 应答里是私网 IP，客户端连不上** | 服务器在 NAT/云主机后，未配置 `pasv_address` | 抓包看 `227` 内容；vsftpd 配 `pasv_address=公网IP`；或改用 `EPSV`（229 应答不含 IP） |
| **530 Login incorrect** | 密码错误；用户在 `/etc/ftpusers` 黑名单；`userlist` 白名单未包含；匿名被禁用 | 看 `/var/log/vsftpd.log` 与 `/var/log/secure`；确认 shell 在 `/etc/shells` 中 |
| **500 OOPS: vsftpd: refusing to run with writable root inside chroot()** | 开了 `chroot_local_user` 但家目录可写 | 加 `allow_writeable_chroot=YES`，或把家目录本身设为不可写、在其下建可写子目录 |
| **550 Permission denied / Failed to open file** | 文件系统权限不足；SELinux 拦截；`write_enable=NO`；磁盘配额满 | `ls -l` 检查属主与权限；`getenforce` 与 `ausearch -m avc`；`df -h` |
| **文件传完但打不开 / 大小不对** | 用了 ASCII 模式传二进制文件，CRLF 被转换 | 传输前必须 `binary` / `TYPE I`；对比本地与远端 `SIZE`、md5 |
| **421 Service not available / Timeout** | 空闲超时被踢；`max_clients` 或 `max_per_ip` 达上限；服务未启动 | 调 `idle_session_timeout`；客户端定期 `NOOP` 保活；`ss -ltnp` 确认服务在听 |
| 大文件传到一半断，且传完收不到 226 | 控制连接长时间空闲被 NAT 会话表或防火墙清除 | 开启 TCP keepalive；缩短 NAT 会话超时排查；客户端启用 keepalive 选项 |
| **FTPS 能登录但传文件失败** | 加密后防火墙 ALG 无法解析 `227` 应答，无法自动放行数据端口 | 必须**手工**放行完整的被动端口范围；或关闭 ALG 后静态放行 |
| FTPS 报 `SSL connection reuse` 错误 | 部分客户端不复用控制连接的 TLS 会话到数据连接 | 服务端设 `require_ssl_reuse=NO` |
| 匿名登录成功但看不到任何文件 | 匿名根目录为空；权限不允许列目录 | 检查 `anon_root` 指向与目录权限（需 r-x） |
| Windows 资源管理器/浏览器打不开 FTP | 浏览器已彻底移除 FTP 支持（Chrome 95+、Firefox 90+） | 改用 FileZilla、WinSCP、lftp 等专用客户端 |
| 中文文件名乱码 | 服务端与客户端字符集不一致；未启用 UTF-8 | `FEAT` 查看是否支持 `UTF8`；客户端 `OPTS UTF8 ON`；lftp 设 `set ftp:charset gbk` |
