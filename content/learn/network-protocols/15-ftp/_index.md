...
- **打电话搬箱子**：FTP 像搬家时一边打电话指挥（控制连接，永远在线，只说"下一步搬什么"），一边临时叫来一个搬运工搬箱子（数据连接，搬完就走）。指挥线始终占着，搬运工随叫随到、干完就撤——所以你能一边搬着货，一边在电话里喊"停！别搬了"（发 `ABOR` 中止）。
- **银行双窗口**：一个窗口只收指令、不碰钱（控制连接，21 端口），另一个窗口专搬现金、不接指令（数据连接）。麻烦之处在于：第二个窗口的"门牌号"是第一个窗口里口头报出来的，所以隔着墙（NAT/防火墙）的搬运工常常找不到门。

## 它解决什么问题

为什么没有它，网络就"缺了一块"：在 1971 年 FTP 诞生的年代，异构主机之间连"文件是什么"都没有共识——不同机器换行符不同、文件结构不同、还互相不认识。FTP 要解决的正是"让两台互不相同的电脑能可靠地互相递文件、列目录、续传"。
...
4. **【文本命令+三位数字应答】**：命令如 `USER`、`RETR`；应答如 `230 Login successful`。每位数字都有明确含义，便于程序解析。
5. **【明文是致命缺陷】**：用户名、密码、命令、文件内容全部明文。RFC 959 设计于没有安全威胁意识的年代，今天**不应在公网使用裸 FTP**。
6. **【安全演进双路径】**：`FTPS`（FTP over TLS，RFC 4217，仍是 FTP）与 `SFTP`（SSH File Transfer Protocol，完全不同的协议，走 SSH 22 端口）。二者名字像但毫无关系。
...
>
> 初学者最常踩的坑：
> - **以为主动/被动"差不多"**：主动模式由服务器从 20 端口**反向连客户端**，在 NAT 普及、主机防火墙默认拒绝入站的今天几乎必然失败（报 `425`）。现代客户端一律用被动模式，别在 NAT 后硬试主动。
> - **忘了先发 `TYPE I`**：用默认的 ASCII 模式传 `.zip`/`.exe`/`.jpg`，文件中恰好的 `0x0D 0x0A` 会被"转换"，导致文件损坏、大小对不上。除明确是纯文本外，一律先 `binary`。
> - **以为 FTPS 就是 SFTP**：名字像但毫无关系——FTPS 仍是双连接 FTP + TLS（21/990），SFTP 是 SSH 子系统、单连接 22 端口。选错了协议和端口都对不上。
