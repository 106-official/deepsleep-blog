...
把 SMTP 想象成**邮局的投递系统**：你把写好的信塞进信封，在信封外面写上寄件人和收件人（这就是 SMTP 的信封 `MAIL FROM` / `RCPT TO`），然后交给邮局。邮局只负责一站一站地把信送到对方邮局的收发室，**不负责替你开信箱取信**。

而信封上的地址和信纸里落款写的名字可以完全不一样——这正是伪造发件人显示名的原理，也是后来必须加装 SPF/DKIM/DMARC 这几把"防伪锁"的原因。
...
>
> ⚠️ 初学者最常踩的坑：把**信封发件人**（`MAIL FROM:`）和**报文头部** `From:` 当成同一个东西。它们相互独立，头部 `From:` 可以随便写，这才是钓鱼邮件能"显示成银行"的原因。另外别忘了 4xx 是临时失败会自动重试、5xx 才是永久退信，两者的处理方式完全不同。
