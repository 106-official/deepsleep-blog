...
**telnet 手工会话（明文 110）**：`+OK` → `CAPA`（见 TOP/UIDL/STLS）→ `USER`(ok) → `PASS 授权码`(+OK) → `STAT`(+OK 2 3210) → `LIST` → `UIDL` → `TOP 1 5`（头部+前5行）→ `RETR 1`（全文）→ `DELE 1` → `QUIT`（此刻真删）。

**Python poplib**：`poplib.POP3_SSL(host,995).user().pass_()` → `stat()`/`uidl()`/`retr(n)`；`dele(n)` 只打标记，`quit()` 才提交删除。

**服务端（Dovecot）**：`doveconf -n | grep -i pop3`；`doveadm who alice`（查会话）；`doveadm kick alice`（踢残留释放锁）。

## 3. 常见故障与排错

| 现象 | 可能原因 | 排查 |
|------|---------|------|
| 超时无 `+OK` | 110/995 被拦截；服务未起 | `Test-NetConnection -Port 995`；`ss -lntp \| grep -E '110\|995'` |
| `-ERR authentication failed` | 用登录密码非**授权码**；未开 POP3 | 网页版开启并重新生成授权码；用户名写**完整邮箱** |
| `-ERR mailbox is locked` | 上会话未 `QUIT` 锁残留；多端同收 | 等 5–10 分钟；`doveadm kick 用户` |
| 反复重复下载 | UID 表丢失（重装/换机）；`DELE` 未生效 | 开"保留副本+记录 UIDL"；确认 `QUIT` 成功；核对 UIDL 两次一致 |
| 删了下次还在 | 会话异常中断未进 UPDATE；服务端只读 | 抓包确认发出 `QUIT` 并收 `+OK` |
| 手机删了电脑还有 | **POP3 不同步**，各端独立下载 | 协议固有限制，需同步改 IMAP |
| 正文被截断 | 未处理点填充 | 按 RFC 1939 去点；以独占一行 `.` 判定结束 |
| `-ERR command not supported` | 服务端未实现 TOP/UIDL | 先 `CAPA`；无 UIDL 只能退化全下载 |
| 只收到部分邮件 | POP3 只暴露 INBOX | 需访问其它文件夹必须 IMAP |
