...
>
> 初学者最常踩的坑：**手工敲命令时忘了加标签**，服务器只会回一句 `BAD`，让人摸不着头脑——IMAP 每条命令都必须以唯一标签开头。另外两个高频坑：一是拿会随删除而重排的**序号**当稳定 ID 用（编程一律用 UID）；二是以为 `STORE +FLAGS \Deleted` 就删掉了，其实还得 `EXPUNGE` 才真删。
