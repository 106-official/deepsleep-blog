...
IMAP 像是**云端网盘里的文件夹**，而 POP3 像是把文件全部拷进 U 盘带走。用网盘时，你在手机上把某个文件挪进"已归档"，电脑上打开看到的也是挪好的样子——因为大家看的是同一份原件。

再打个比方：POP3 是把书从图书馆借回家，IMAP 是**在图书馆里看书**——你贴的书签、做的分类，下次换个座位（换台设备）来看，全都还在原处。
...
>
> ⚠️ 初学者最常踩的坑：**手工敲命令时忘了加标签**，服务器只会回一句 `BAD`，让人摸不着头脑——IMAP 每条命令都必须以唯一标签开头。另外两个高频坑：一是拿会随删除而重排的**序号**当稳定 ID 用（编程一律用 UID）；二是以为 `STORE +FLAGS \Deleted` 就删掉了，其实还得 `EXPUNGE` 才真删。
