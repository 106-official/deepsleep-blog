...
> ⚠️ 初学者最常踩的坑：一是把 `NXDOMAIN`（域名根本不存在）和 `NOERROR` 但 `ANSWER=0`（域名存在、只是没你要的那种记录，比如只有 A 没有 AAAA）混为一谈，二者排查路径完全不同；二是以为"改了 DNS 记录立刻生效"，其实各级缓存按旧记录的剩余 TTL 生效，没到时间就是不变——改之前得先降 TTL。三是以为 DNSSEC 等于加密，其实它只验证答案没被篡改，查询内容照样明文可见，机密性得靠 DoT/DoH。
