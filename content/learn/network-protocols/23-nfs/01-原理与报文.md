...
每个 RPC 请求用 `{程序号, 版本号, 过程号}` 唯一定位要调用的函数：
...
### NFSv4 的 COMPOUND（RFC 7530）

v4 **只有两个 RPC 过程**：`NULL(0)` 和 **`COMPOUND(1)`**。所有实际操作都是 COMPOUND 里的"操作数组"：
...
NFS **不保证实时一致性**。v3 的保证只有 **close-to-open**：
