...
想象你请了一位助理坐在千里之外的那台电脑前：你说"鼠标往左移、点这里、敲这几个字"，他照做；他则不停地把屏幕的变化描述给你听——而且很聪明，不会念"每一个像素长什么样"，只会说"这里画一个矩形、那块内容整体往上滚了一行"。你脑子里据此把画面还原出来。

更妙的是，你们之间还架了好几条专线：一条传剪贴板、一条传声音、一条把你桌上的打印机和 U 盘"借"给那台远程电脑用。这些并行的专线，就是 RDP 的**虚拟通道**。
...
| 特征 | 说明 |
|------|------|
| **多通道复用（Multi-Channel）** | 【一条连接跑多条道】基于 T.125 MCS，在一条连接内复用多个虚拟通道：`rdpdr`（设备重定向）、`cliprdr`（剪贴板）、`rdpsnd`（音频）、`drdynvc`（动态虚拟通道）等 |
| **图形远程化** | 【传指令不传帧】传输顺序命令（Orders）、位图更新（Bitmap Update）、位图缓存（Bitmap Cache）、Surface Commands；新版支持 **RemoteFX**、**H.264/AVC 420/444** 编码 |
| **双传输栈** | 【TCP 保对错，UDP 保流畅】TCP 保证可靠（输入、控制、通道协商）；UDP（RDP-UDP，可靠/尽力两种模式 + DTLS）承载图形与音频以降低时延 |
| **安全层可协商** | 【认证可以前置】Standard RDP Security（RC4，已弃用）→ **TLS** → **CredSSP/NLA**（网络级身份验证，连接前先认证，抵御未认证会话耗尽与部分漏洞利用） |
| **设备重定向** | 【本地外设借给远端】剪贴板、驱动器、打印机（Easy Print）、音频输入输出、智能卡、摄像头、USB |
| **会话保持与重连** | 【掉线不丢现场】断线后会话在服务端保留（可配置），支持自动重连（Auto-Reconnect Cookie） |
| **服务端有状态** | 【懂 GDI 语义】与 VNC 的"纯像素同步"不同，RDP 服务端理解 Windows GDI 绘图语义，是操作系统深度集成的协议 |
...
1. **[01-原理与报文](01-原理与报文/)** — X.224/MCS 分层如何叠出 RDP、连接序列的 10 个阶段、TPKT 与 PDU 头部字段、虚拟通道机制、安全协商与 NLA 时序图。
2. **[02-实战与排错](02-实战与排错/)** — Wireshark 过滤 `tcp.port==3389 || rdp`、`mstsc` / `xfreerdp` / PowerShell 配置命令、"无法连接/CredSSP 加密数据库修正/黑屏/证书告警"排错，以及与 VNC/SSH/SPICE 的横向对比与速查表。

> 学习建议：先弄懂"**为什么 RDP 要套 X.224 和 MCS 这两层老古董**"（答案在 T.120 会议系统血统里），再看虚拟通道，最后动手抓一次完整连接握手，RDP 的复杂度就基本拆解完了。

> ⚠️ 初学者最常踩的坑：**以为 RDP 的 UDP 端口是 3390**。实际上 UDP 与 TCP 同为 **3389**，3391 是 RD Gateway 的 UDP over RDP 端口。另一个高频误解是"RDP 像录屏一样逐帧传视频"——它优先传的是绘图订单与缓存引用，不是视频流。
