...
> 初学者最常踩的坑：**把 SFTP 和 FTPS 当成一回事**——SFTP 是 SSH 的子系统，走 TCP 22 单端口；FTPS 是 FTP 加 TLS，仍是控制加数据的双通道模型，两者毫无协议关系。另一个高频坑是**公钥登录失败时到处找配置问题**，实际最常见的原因是 `~/.ssh`、`authorized_keys` 或家目录**权限过宽**。
