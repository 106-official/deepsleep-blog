...
| 特征 | 说明 |
|------|------|
| **三层子协议架构** | 传输层协议（加密、完整性、服务端认证）→ 用户认证协议 → 连接协议（多通道复用） |
| **算法全可协商** | 密钥交换（`curve25519-sha256`、`ecdh-sha2-nistp256`、`diffie-hellman-group14/16-sha256`）、主机密钥（`ssh-ed25519`、`rsa-sha2-512`、`ecdsa-sha2-nistp256`）、对称加密（`chacha20-poly1305@openssh.com`、`aes256-gcm@openssh.com`、`aes128-ctr`）、MAC（`hmac-sha2-256-etm@openssh.com`） |
| **前向保密（PFS）** | 使用临时 DH/ECDH 密钥交换，主机私钥泄露也无法解密历史流量；且支持**密钥重协商（rekey）** |
| **多种认证方式** | `publickey`（推荐）、`password`、`keyboard-interactive`（含 OTP/2FA）、`gssapi-with-mic`（Kerberos）、`hostbased`、OpenSSH 证书、FIDO2/U2F（`ed25519-sk`）、`none`（探测） |
| **多路通道复用** | 单连接内并行开 shell、sftp、端口转发、X11；OpenSSH 的 `ControlMaster` 还能让多个 ssh 命令共用一条 TCP 连接 |
| **端口转发三形态** | 本地 `-L`、远程 `-R`、动态 `-D`（SOCKS5） |
| **文件传输** | `scp`（旧，OpenSSH 9.0 起底层改用 SFTP）、`sftp`（子系统，功能完整）、`rsync -e ssh` |
| **主机密钥指纹** | SHA256 指纹 + `known_hosts`；企业可用 SSHFP DNS 记录或 SSH CA 证书替代 TOFU |
