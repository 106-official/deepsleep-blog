...
>
> ⚠️ 初学者最常踩的坑：把"时区差 8 小时"当成"时间不对"（前者 `timedatectl set-timezone` 即可，与 NTP 无关）；以为 Stratum 数字越小一定越准（其实 RTT 300 ms 的远端 Stratum 1 可能不如同机房 Stratum 3）；只配 1~2 个时间源导致无法识别说谎源；忘了"容器不该自己跑 NTP"（共享宿主内核时钟）。
