...
> **最后更新**: 2026-08-14
> **版本**: v5.17
> **状态**: ✅ 生产就绪 | 评论系统正常运行 (Neon PostgreSQL) | 💬 社区系统已上线 (腾讯云 SCF) | 👤 全局个人中心 | 📝 文章板块整合 (learn 风格 sidebar) | 🌗 主题切换圆形扩散动画 | 🐟 SleepTown 首页 sidebar 改造 | 🎮 交互式自我介绍 (/play/me/) | 🃏 CardArena 卡牌对战 (/play/cardarena/) | 👑 CardArena 苏丹宫廷风卡牌样式 | ⚔️ CardArena 极繁深化 | 🎴 CardArena 对战页去框化+二维布局 | 🃏 CardArena 每角色独立卡池 | ✨ CardArena 选卡特效+选首发阶段
...
### 腾讯云 SCF 环境变量 (scf-community-deepsleep 项目) v5.1 新增 · v5.17 迁移
...
> 部署方式: 腾讯云函数 SCF（Web 函数类型），`scf_bootstrap` 启动 + 监听 9000 端口，`makezip.py` 打包 `code.zip` 上传更新。端点 `https://1437998910-loiqxuadw0.ap-shanghai.tencentscf.com`（国内直连可达）。
> 旧 Vercel 版（community-deepsleep.vercel.app）已废弃保留备用，见 `docs/community/vercel-legacy.md`。
...
> 完整请求路径: `https://1437998910-loiqxuadw0.ap-shanghai.tencentscf.com/api/posts`
...
### v5.17 (2026-08-14) - 文档整合与同步

**变更内容**:
- ✅ **全部项目/子项目文档整合**至 `c:\Users\26516\Desktop\n8n\docs\` 统一目录（不再随 blog-static 仓库存放）
  - `docs/blog/`：README / PROJECT_CONTEXT / PROJECT_DOCUMENTATION / sections（板块文档 + cpa-source）
  - `docs/community/`：SCF 当前版说明 + Vercel 废弃版说明
  - `docs/scf-lixin-ask/`、`docs/remote-control/`、`docs/waline/`：子项目 README
  - `docs/superpowers/`：设计文档（plans + specs，原样保留）
- ✅ **删除废弃的本地 Dify 目录**（docker volumes，已不再使用）
- ✅ **同步社区后端 SCF 迁移现状**：架构/API/部署/诊断命令全部更新为腾讯云 SCF 端点，旧 Vercel 端点标注为废弃备用
- ✅ **各子项目补齐 README**（此前缺失）

**Files Modified**:
| 文件 | 变更 |
|------|------|
| `docs/blog/README.md` | 重写：v5.4 → v5.17、真实 GitHub/Waline/SCF 架构、板块清单 |
| `docs/blog/PROJECT_CONTEXT.md` | 版本 v5.16→v5.17、架构章节 SCF 化、部署/诊断命令更新、演进表加 v5.17 |
| `docs/blog/PROJECT_DOCUMENTATION.md` | 版本 v5.16→v5.17、部署环境加 SCF 章节、Community API 更新 |
| `docs/README.md` | 新增：文档中心索引 |
| `docs/community/README.md` | 新增：scf-community-deepsleep 说明 |
| `docs/community/vercel-legacy.md` | 新增：Vercel 废弃版说明 |
| `docs/scf-lixin-ask/README.md` | 新增 |
| `docs/remote-control/README.md` | 新增 |
| `docs/waline/README.md` | 定制化：官方模板 → DeepSleep 评论系统说明 |
